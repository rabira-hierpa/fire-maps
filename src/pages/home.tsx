...
import { DB_COLLECTION, TOKEN } from "../utils/db";
  ...
  const saveFeaturesToDb = async (newFeature: IFeature[]) => {
    ...
    try {
      ...
      const newData = {
        ...
        geometry: {
          ...newPolygon.geometry,
          // Firebase does not support Array[Array[Number]] so had to flat out the coordinates using join
    ...
    <div className="">
      ...
      >
        <DrawControl
          ...
          onDelete={onDelete}
        />
        <div className="w-[29px] h-[29px] z-10 relative top-20 ml-2">
          <span className="bg-white p-2 border-1 rounded-lg text-md cursor-pointer hover:bg-gray-200">
            <Link to="3d-map">3D</Link>
          </span>
        </div>
